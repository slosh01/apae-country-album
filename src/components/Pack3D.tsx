...
import { useGLTF, useAnimations, PerspectiveCamera, Environment, Float, Center, SpotLight, Html } from '@react-three/drei';
...
function LoadingUI() {
  return (
    <Html center>
      <div className="flex flex-col items-center gap-3 w-max">
        <Loader2 className="w-8 h-8 text-[#422422] animate-spin" />
        <span className="text-[#422422] font-alfa text-[10px] uppercase tracking-widest opacity-60">
          Aguarde para abrir
        </span>
      </div>
    </Html>
...
export default function Pack3D({ modelPath, onOpenComplete, isOpeningExternally }: any) {
  ...
  // Pre-carrega e detecta quando terminou
  useEffect(() => {
    setIsLoaded(false);
  ...
  const bind = useGesture({
    onDrag: ({ offset: [x, y] }) => {
      if (completed || isFinalizing || !isLoaded) return;
  ...
  const handleSliderChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (completed || isFinalizing || isOpeningExternally || !isLoaded) return;
      ...
      <div className="w-full flex-1 relative touch-none" {...(bind() as any)}>
        <Canvas shadows gl={{ antialias: true, powerPreference: "high-performance", alpha: true }} camera={{ position: [0, 0, 10], fov: 18 }}>
          <Suspense fallback={<LoadingUI />}>
            ...
            />
            {/* Componente invisível apenas para avisar o React que terminou de carregar */}
            <OnLoad onLoaded={() => setIsLoaded(true)} />
          </Suspense>
        </Canvas>
      </div>

      {/* Slider só aparece após o modelo estar 100% carregado */}
      {isLoaded && !isFinalizing && !completed && !isOpeningExternally && (
        <div className="w-[85%] max-w-[280px] p-2 z-[100] mb-[5vh]">
          <div className="flex flex-col items-center gap-6">
            <span className="text-[#422422] font-alfa text-[8px] uppercase tracking-[0.3em] opacity-80 animate-pulse">Arraste para abrir</span>
...
// Helper para avisar quando o Suspense terminou
function OnLoad({ onLoaded }: { onLoaded: () => void }) {
  useEffect(() => {
    onLoaded();
  }, [onLoaded]);
  return null;
